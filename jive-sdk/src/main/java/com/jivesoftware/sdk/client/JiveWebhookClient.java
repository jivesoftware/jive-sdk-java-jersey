...
import com.jivesoftware.sdk.api.webhook.data.Webhook;
...
public class JiveWebhookClient extends JiveAPIClient {
    ...
    private URI buildAbsoluteURIFromRelative(String url) throws WebhookClientException {
        if (url != null) {
            if (url.indexOf("/") == 0) {
                return UriBuilder.fromUri(jiveAddOnConfig.getClientUrl()).port(jiveAddOnConfig.getPort()).path(url).build();
            } // end if
